...
const AdminDashboard = () => {
  ...
  if (currentUser.role !== "admin") {
    return <div className="max-w-3xl mx-auto px-4 py-16 text-center text-gray-500">Admins only.</div>
  }
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`pb-2 text-sm font-medium whitespace-nowrap ${
              tab === t.key ? "border-b-2 border-brand-600 text-brand-600" : "text-gray-400"
            }`}
          >
            {t.label}
          </button>
      ...
      {tab === "users" && (
        <div className="flex flex-col gap-3">
          {users.map((u) => (
            <div key={u.id} className="flex items-center justify-between border-b border-gray-50 pb-3">
              <div>
                <Link to={`/profile/${u.username}`} className="font-medium hover:text-brand-600">
                  {u.username}
                </Link>
                <p className="text-xs text-gray-400">{u.email} · {u.postCount} posts · joined {moment(u.created_at).format("MMM YYYY")}</p>
              </div>
              <div className="flex items-center gap-3">
                <select
                  ...
                  className="text-sm border border-gray-200 rounded-md px-2 py-1 disabled:opacity-50"
                >
        ...
        <div className="flex flex-col gap-3">
          {posts.map((p) => (
            <div key={p.id} className="flex items-center justify-between border-b border-gray-50 pb-3">
              <div>
                <Link to={`/post/${p.slug}`} className="font-medium hover:text-brand-600">
                  {p.title}
                </Link>
              ...
              <button onClick={() => handleDeletePost(p.id)} className="text-sm text-red-500 hover:underline">
                Delete
              </button>
            </div>
          ))}
      ...
      {tab === "comments" && (
        <div className="flex flex-col gap-3">
          {comments.map((c) => (
            <div key={c.id} className="flex items-start justify-between border-b border-gray-50 pb-3 gap-4">
              <div>
                <p className="text-sm">{c.content}</p>
                <p className="text-xs text-gray-400 mt-1">
                  {c.username} on{" "}
                  <Link to={`/post/${c.postSlug}`} className="hover:text-brand-600">
                    {c.postTitle}
                  </Link>{" "}
        ...
        <div className="flex flex-col gap-4">
          <div className="flex gap-2">
            <input
              ...
              className="border border-gray-200 rounded-md px-3 py-1.5 text-sm outline-none focus:border-brand-400"
            />
            ...
            {categories.map((c) => (
              <div key={c.id} className="flex items-center justify-between border-b border-gray-50 pb-2">
                ...
                <div className="flex gap-3 text-sm">
                  {editingCat === c.id ? (
                    <button onClick={() => saveEditCategory(c.id)} className="text-brand-600 hover:underline">
                      Save
                    </button>
                  ) : (
                    <button onClick={() => startEditCategory(c)} className="text-brand-600 hover:underline">
                      Edit
                    </button>
                  )}
